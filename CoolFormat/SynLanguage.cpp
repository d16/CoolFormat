...
{	
	...
	case SYN_AUTOIT:
		...
		break;
	case SYN_BASH:
		strWords = _T("alias,bg,bind,break,builtin,case,cd,command,continue,declare,dirs,disown,do,done,echo,elif,else,enable,esac,eval,exec,exit,export,fc,fg,fi,for,function,getops,hash,help,history,if,in,jobs,kill,let,local,logout,popd,pushd,pwd,read,r,adonly,return,select,set,shift,suspend,test,then,time,times,trap,type,typeset,ulimit,umask,unalias,unset,until,wait,while,");
		break;
    case SYN_BATCH:
